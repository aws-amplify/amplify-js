 ...
 */
export function categorizeRekognitionBlocks(blocks: Rekognition.TextDetectionList): IdentifyTextOutput {
 ...
 * @return {IdentifyTextOutput} -  Object that categorizes each block and its information.
 */
export function categorizeTextractBlocks(blocks: Textract.BlockList): IdentifyTextOutput {
    // Skeleton IdentifyText API response. We will populate it as we iterate through blocks.
    const response: IdentifyTextOutput = {
        text: {
            fullText: '',
 ...
 */
export function constructTable(table: Textract.Block, blockMap: { [key: string]: Textract.Block }): Table {
