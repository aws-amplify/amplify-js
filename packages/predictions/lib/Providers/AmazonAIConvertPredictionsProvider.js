...
var eventBuilder = new eventstream_marshaller_1.EventStreamMarshaller(
...
var AmazonAIConvertPredictionsProvider = /** @class */ (function(_super) {
	...
	AmazonAIConvertPredictionsProvider.prototype.convertTextToSpeech = function(
		input
	) {
		var _this = this;
		return new Promise(function(res, rej) {
	...
	AmazonAIConvertPredictionsProvider.prototype.convertSpeechToText = function(
		input
	) {
		var _this = this;
		return new Promise(function(res, rej) {
				...
				return __generator(this, function(_h) {
					switch (_h.label) {
						...
						case 1:
							...
							(_f = input.transcription),
								(source = _f.source),
								(_g = _f.language),
								(language = _g === void 0 ? languageCode : _g);
	...
	AmazonAIConvertPredictionsProvider.serializeDataFromTranscribe = function(
		message
	) {
		var decodedMessage = '';
		var transcribeMessage = eventBuilder.unmarshall(Buffer.from(message.data));
		var transcribeMessageJson = JSON.parse(
	...
	AmazonAIConvertPredictionsProvider.prototype.sendDataToTranscribe = function(
		_a
	) {
		var _this = this;
		...
		return new Promise(function(res, rej) {
			...
			connection.onmessage = function(message) {
				try {
					var decodedMessage = AmazonAIConvertPredictionsProvider.serializeDataFromTranscribe(
						message
					);
					if (decodedMessage) {
	...
	AmazonAIConvertPredictionsProvider.prototype.openConnectionWithTranscribe = function(
		_a
	) {
		var _this = this;
	...
	AmazonAIConvertPredictionsProvider.prototype.generateTranscribeUrl = function(
		_a
	) {
		...
		var signedUrl = core_1.Signer.signUrl(
			url,
			credentials,
			{ region: region, service: 'transcribe' },
			300
		);
		return signedUrl;
	};
