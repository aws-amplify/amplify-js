...
interface StorageMobileHubConfig {
	Storage: {
		AWSS3: {
			bucket: string;
			region: string;
...
export type S3ProviderGetConfig =
	| (StorageOptions & {
			download?: boolean;
			track?: boolean;
			expires?: number;
...
export type S3ProviderRemoveConfig = (StorageOptions & { bucket?: string }) | StorageMobileHubConfig;

export type S3ProviderRemoveOutput = DeleteObjectCommandOutput;

export type S3ProviderListConfig =
	| (StorageOptions & {
			bucket?: string;
			maxKeys?: number;
	  })
...
export type S3ProviderCopyConfig =
	| (StorageOptions & {
			cancelTokenSource?: CancelTokenSource;
