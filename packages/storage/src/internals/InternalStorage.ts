...
export class InternalStorageClass {
	...
	private updateRequestToBeCancellable(request: Promise<any>, abortController: AbortController) {
	...
	public cancel(
		...
		const abortController = this._abortControllerMap.get(request as Promise<any>);
		if (abortController) {
	...
	public copy<T extends StorageProviderWithCopy = AWSS3Provider>(
		...
		const plugin = this._pluggables.find(pluggable => pluggable.getProviderName() === provider);
		if (plugin === undefined) {
		...
		if (typeof plugin.copy !== 'function') {
			return Promise.reject(`.copy is not implemented on provider ${plugin.getProviderName()}`) as StorageCopyOutput<T>;
		}
	...
	public getProperties<T extends StorageProvider | { [key: string]: any }>(
		...
		const responsePromise = plugin?.getProperties(
			key,
			config,
			getStorageUserAgentValue(StorageAction.GetProperties, customUserAgentDetails)
		);
		this.updateRequestToBeCancellable(responsePromise, abortController);
