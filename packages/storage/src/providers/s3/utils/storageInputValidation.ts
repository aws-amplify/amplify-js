...
export const validateStorageOperationInput = (input: Input, identityId: string = '') => {
	if (isInputWithPath(input)) {
		const { path } = input;
		return { inputType: STORAGE_INPUT_TYPES.PATH, objectKey: typeof path === 'string' ? path : path({identityId})  };
