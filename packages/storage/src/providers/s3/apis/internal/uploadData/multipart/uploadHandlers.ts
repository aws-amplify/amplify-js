...
export const getMultipartUploadHandlers = (
	...
	const startUpload = async (): Promise<ItemWithKey | ItemWithPath> => {
		...
		if (!inProgressUpload) {
			...
			console.log(`[multipart] createMultipartUpload → uploadId: ${uploadId}`);
		}
		...
		const totalParts = Math.ceil(size / calculatePartSize(size));
		const onPartUploadCompletion = (
			...
			console.log(`[multipart] uploadPart ${partNumber}/${totalParts} complete (ETag: ${eTag})`);
		};
		...
		console.log(`[multipart] completeMultipartUpload → ETag: ${eTag}`);
...
const resolveAccessLevel = (
	amplifyInstance: AmplifyClassV6,
	accessLevel?: StorageAccessLevel,
) =>
	accessLevel ??
	amplifyInstance.libraryOptions.Storage?.S3?.defaultAccessLevel ??
	DEFAULT_ACCESS_LEVEL;
