...
import { readFile } from '../../../../src/providers/s3/utils/readFile.native';
...
describe('readFile', () => {
	...
	it('should fallback to readAsDataURL when readAsArrayBuffer is not supported', async () => {
		const mockFile = new Blob(['test content']);
		const mockBase64Data = 'base64encodeddata';
		const mockDataURL = `data:application/octet-stream;base64,${mockBase64Data}`;
		...
		expect(Buffer.from).toHaveBeenCalledWith(mockBase64Data, 'base64');
	});
