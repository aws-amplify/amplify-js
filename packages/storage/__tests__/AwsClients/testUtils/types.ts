...
export type XhrSpy = Writeable<XMLHttpRequest> & {
	uploadListeners: Partial<{
		[name in keyof XMLHttpRequestEventTargetEventMap]: Array<(event: XMLHttpRequestEventTargetEventMap[name]) => void>;
	}>;
	listeners: Partial<{
		[name in keyof XMLHttpRequestEventMap]: Array<(event: XMLHttpRequestEventMap[name]) => void>;
	}>;
};
