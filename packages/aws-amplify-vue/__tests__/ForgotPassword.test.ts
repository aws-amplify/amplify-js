...
import ForgotPassword from '@/components/authenticator/ForgotPassword.vue';
...
describe('ForgotPassword', () => {
	...
	describe('...when it is mounted without props...', () => {
		...
		it('...should call Auth.forgotPassword when submit method is called', () => {
			wrapper.vm.submit();
			expect(wrapper.vm.$Amplify.Auth.forgotPassword).toBeCalledWith(
				wrapper.vm.forgotPwUsername
			);
		});
		...
		it('...should call Auth.forgotPasswordSubmit when verify method is called', () => {
			wrapper.vm.verify();
			expect(wrapper.vm.$Amplify.Auth.forgotPasswordSubmit).toBeCalledWith(
				wrapper.vm.forgotPwUsername,
				wrapper.vm.password,
				wrapper.vm.code
			);
