...
import PhotoPicker from '@/components/storage/PhotoPicker.vue';
...
describe('PhotoPicker', () => {
	...
	describe('...when the methods are not mocked...', () => {
		...
		it('...should have a upload method that calls Amplify.Auth.PhotoPicker', () => {
			...
			wrapper.vm.upload();
			expect(wrapper.vm.$Amplify.Storage.put).toHaveBeenCalledWith(
				'testPath',
				{ type: 'fakeFile' },
				{ level: 'private', contentType: 'fakeType' }
			);
		});
	});
