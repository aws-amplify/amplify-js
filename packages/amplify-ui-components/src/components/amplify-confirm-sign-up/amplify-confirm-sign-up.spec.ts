...
describe('amplify-confirm-sign-up spec:', () => {
  describe('Component logic ->', () => {
    ...
    it('should render `headerText` to `Confirm Sign up` by default', () => {
      expect(confirmSignUp.headerText).toEqual(I18n.get(AuthMessages.CONFIRM_SIGN_UP_HEADER_TEXT));
    });

    it('should render `submitButtonText` to `Confirm` by default', () => {
      expect(confirmSignUp.submitButtonText).toEqual(I18n.get(AuthMessages.CONFIRM_SIGN_UP_SUBMIT_BUTTON_TEXT));
    });
  });
