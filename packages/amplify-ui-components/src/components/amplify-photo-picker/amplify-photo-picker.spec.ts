...
describe('amplify-photo-picker spec:', () => {
  describe('Component logic ->', () => {
    ...
    it('`headerTitle` should be defined by default', () => {
      expect(amplifyPhotoPicker.headerTitle).toBe(I18n.get(Translations.PHOTO_PICKER_TITLE));
    });

    it('`headerHint` should be defined by default', () => {
      expect(amplifyPhotoPicker.headerHint).toBe(I18n.get(Translations.PHOTO_PICKER_HINT));
    });

    it('`placeholderHint` should be defined by default', () => {
      expect(amplifyPhotoPicker.placeholderHint).toBe(I18n.get(Translations.PHOTO_PICKER_PLACEHOLDER_HINT));
    });

    it('`buttonText` should be defined by default', () => {
      expect(amplifyPhotoPicker.buttonText).toBe(I18n.get(Translations.PHOTO_PICKER_BUTTON_TEXT));
    });
