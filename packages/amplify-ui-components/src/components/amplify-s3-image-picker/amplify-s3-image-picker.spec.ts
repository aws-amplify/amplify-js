...
describe('amplify-s3-image spec:', () => {
  describe('Component logic ->', () => {
    ...
    it('`headerTitle` should have value by default', () => {
      expect(amplifyS3ImagePicker.headerTitle).toBe(I18n.get(Translations.IMAGE_PICKER_TITLE));
    });

    it('`headerHint` should have value by default', () => {
      expect(amplifyS3ImagePicker.headerHint).toBe(I18n.get(Translations.IMAGE_PICKER_HINT));
    });
    ...
    it('`buttonText` should have value by default', () => {
      expect(amplifyS3ImagePicker.buttonText).toBe(I18n.get(Translations.IMAGE_PICKER_BUTTON_TEXT));
    });
  });
