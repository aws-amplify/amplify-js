...
describe('amplify-confirm-sign-in spec:', () => {
  describe('Component logic ->', () => {
    ...
    it('should render `headerText` to `Confirm SMS Code` by default', () => {
      expect(confirmSignIn.headerText).toEqual(I18n.get(AuthMessages.CONFIRM_SMS_CODE));
    });

    it('should render `submitButtonText` to `Confirm` by default', () => {
      expect(confirmSignIn.submitButtonText).toEqual(I18n.get(AuthMessages.CONFIRM));
    });
  });
