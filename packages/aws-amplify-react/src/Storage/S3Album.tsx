...
export interface IS3AlbumProps {
    contentType?: string | ((item) => string);
    fileToKey?: string | object | ((info: {name: string, size: number, type: string}) => string);
    ...
    onLoad?: (data) => void;
    onPick?: (data) => void;
    onSelect?: (item, selectedItems: any[]) => void;
    path?: string;
    picker?: boolean;
    pickerTitle?: string;
    select?: boolean;
    sort?: string | ((items) => any[]);
    theme?: IAmplifyTheme;
    track?: boolean;
    translateItem?: string | ((info: { type, key, content}) => string | any);
    ts?: number;
}
...
export default class S3Album extends Component<IS3AlbumProps, IS3AlbumState> {
    ...
    marshal(list) {
        ...
        list.forEach(item => {
            ...
            item.contentType = typeof contentType === 'string' ? contentType : contentType(item);
    ...
    sort(list) {
        ...
        if (['string', 'undefined'].includes(typeof sort)) {
