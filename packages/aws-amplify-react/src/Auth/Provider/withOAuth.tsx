...
export interface IWithOAuthProps {
    label?: string;
    oauth_config?: {
        domain: string, 
    ...
    onStateChange?: (state, authUser) => void;
    theme?;
}

export default function withOAuth(Comp, options?) {
    return class extends Component<IWithOAuthProps> {
        constructor(props) {
            super(props);
        ...
        signIn() {
            ...
            const url = 'https://' + domain 
                ...
                + '&client_id=' + (authOptions.ClientId || Auth.configure().userPoolWebClientId);
