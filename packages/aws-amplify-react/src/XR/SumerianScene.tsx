...
export interface ISumerianSceneProps {
  sceneName: string;
}

interface ISumerianSceneStats {
...
class SumerianScene extends React.Component<ISumerianSceneProps, ISumerianSceneStats> {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    if (XR.isSceneLoaded(this.props.sceneName)) {
      ...
      if (XR.isVRCapable(this.props.sceneName)) {
        enterVRButton
          = <IconButton variant="enter-vr" tooltip="Enter VR" onClick={() => XR.enterVR(this.props.sceneName)} />;
      }
