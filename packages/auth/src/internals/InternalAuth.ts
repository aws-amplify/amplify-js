...
export class InternalAuthClass {
	...
	wrapRefreshSessionCallback = (callback: NodeCallback.Any) => {
		const wrapped: NodeCallback.Any = (error, data) => {
			if (data) {
				dispatchAuthEvent({ event:'tokenRefresh', data: undefined, message: 'New token retrieved'});
			} else {
				dispatchAuthEvent({
					...
					onSuccess: data => {
						if (!isLoggedIn) {
							dispatchAuthEvent({
								event: 'signIn',
								data: internalUser,
								message: `A user ${internalUser.getUsername()} has been signed in`,
							});
						}
						dispatchAuthEvent({
							event: 'verify',
							data: internalUser,
							message: `A user ${internalUser.getUsername()} has been verified`,
						});
								...
								(err, result: string) => {
									if (err) {
										...
										dispatchAuthEvent({
											event: 'userDeleted',
											data: result,
											map: 'The authenticated user has been deleted.',
										});
