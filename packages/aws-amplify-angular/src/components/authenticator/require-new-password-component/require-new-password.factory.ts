...
export class RequireNewPasswordComponent implements OnInit, OnDestroy {
  ...
  loadComponent() {
    ...
    let requireNewPasswordComponent = this.framework && this.framework.toLowerCase() === 'ionic' ? ComponentMountMap('requireNewPasswordIonic',{authState: this.authState}) : ComponentMountMap('requireNewPasswordCore', {authState: this.authState});
