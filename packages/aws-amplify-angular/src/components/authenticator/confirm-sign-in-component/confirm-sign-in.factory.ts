...
export class ConfirmSignInComponent implements OnInit, OnDestroy {
  ...
  loadComponent() {
    ...
    let authComponent = this.framework && this.framework.toLowerCase() === 'ionic' ? ComponentMountMap('confirmSignInIonic',{authState: this.authState}) : ComponentMountMap('confirmSignInCore', {authState: this.authState});
