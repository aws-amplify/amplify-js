...
export class ConfirmSignUpComponent implements OnInit, OnDestroy {
  ...
  loadComponent() {
    ...
    let authComponent = this.framework && this.framework.toLowerCase() === 'ionic' ? new ComponentMount(ConfirmSignUpComponentIonic,{hide: this.hide, authState: this.authState}) : new ComponentMount(ConfirmSignUpComponentCore, {hide: this.hide, authState: this.authState});
