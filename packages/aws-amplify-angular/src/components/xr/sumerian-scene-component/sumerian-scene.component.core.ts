...
const template = `
<div id="sumerian-scene-container" class={{amplifyUI.sumerianSceneContainer}} data-test="${
    sumerianScene.container
}">
  <div id="sumerian-scene-dom-id" class={{amplifyUI.sumerianScene}} data-test="${
      sumerianScene.sumerianScene
  }">
    <sumerian-scene-loading-core *ngIf="loading" loadPercentage={{loadPercentage}} sceneName={{sceneName}} sceneError={{sceneError}} data-test="${
        sumerianScene.loading
    }"></sumerian-scene-loading-core>
  </div>
...
export class SumerianSceneComponentCore implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        if (!this.amplifyService.xr()) {
            throw new Error(
                'XR module not registered on AmplifyService provider'
            );
        }
    ...
    toggleVRPresentation() {
        try {
            ...
            this.logger.error(
                'Unable to start/stop WebVR System: ' + e.message
            );
            return;
        }
