...
import {
	ReconnectEvent,
	ReconnectionMonitor,
} from '~/src/utils/ReconnectionMonitor';
...
export interface ObserverQuery {
	...
	subscriptionReadyCallback?(...args: any[]): unknown;
	subscriptionFailedCallback?(...args: any[]): unknown;
	...
	subscribe(
		...
		return new Observable(observer => {
			if (!options || !appSyncGraphqlEndpoint) {
				...
				const reconnectSubscription = new Observable(reconnectionObserver => {
					this.reconnectionMonitor.addObserver(reconnectionObserver);
				}).subscribe(() => {
					startSubscription();
				});
	...
	private async _startSubscriptionWithAWSAppSyncRealTime({
		...
		this.subscriptionObserverMap.set(subscriptionId, {
			...
			startAckTimeoutId: setTimeout(() => {
				// TODO(eslint): remove this linter suppression with refactor.
		...
		if (subscriptionObserver) {
			...
			if (subscriptionState === SUBSCRIPTION_STATUS.PENDING) {
				return new Promise((resolve, reject) => {
					const {
						observer,
						subscriptionState: subscriptionStateWhenPending,
						variables,
						query,
			...
			await (() => {
				return new Promise((resolve, reject) => {
					if (this.awsRealTimeSocket) {
						...
						const checkAckOk = (ackOkBeingChecked: boolean) => {
							if (!ackOkBeingChecked) {
								this.connectionStateMonitor.record(
