	...
	describe.only('custom endpoints', () => {
		describe.only('non-subscription operations', () => {
			test.only('sets authMode to "none" by default', async () => {
				...
				Amplify.configure({
					API: {
						GraphQL: {
							defaultAuthMode: 'apiKey',
							apiKey: 'FAKE-KEY',
							endpoint: 'https://a-default-appsync-endpoint.local/graphql',
							region: 'north-pole-7',
						},
					},
				});
				...
				console.log('spy call', JSON.stringify(spy.mock.calls, null, 2));
			...
			test('something something IAM auth', async () => {});

			test('something something apiKey auth', async () => {});

			test('something something tokenAuth auth', async () => {});

			test('something something tokenAuth with custom token auth', async () => {});
		});

		describe('subscription operations', () => {
			test('sets authMode to "none" by default', async () => {
				// just need to fill this test out to call POC done and start designing
				// the behavior for the other auth modes.
			});
