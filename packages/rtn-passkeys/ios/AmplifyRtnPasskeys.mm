...
- (void)getPasskey:(JS::NativeAmplifyRtnPasskeys::PasskeyGetOptionsJson &)input
	...
	NSMutableArray<ASAuthorizationPlatformPublicKeyCredentialDescriptor *>
	    *allowedCredentials = [NSMutableArray array];
	...
	if (@available(iOS 16.0, *)) {
		if (input.allowCredentials().has_value()) {
			...
			for (const auto &credential : credentials) {
				...
				[allowedCredentials addObject:pkcDescriptor];
			}
		}
	...
	platformKeyRequest.userVerificationPreference = input.userVerification();
	...
	if ([credential
		...
		NSDictionary *assertionResult = @{
			...
			@"response" : @{
				...
				@"userHandle" : [AmplifyRtnPasskeysBase64UrlTranscoder
				    base64UrlEncode:assertionCredential.userID]
			},
		};
