...
export default class CognitoUser {
  ...
  authenticateUser(authDetails, callback) {
    const authenticationHelper = new AuthenticationHelper(this.pool.getUserPoolId().split('_')[1]);
    ...
    this.client.makeUnauthenticatedRequest('initiateAuth', {
      ...
      const hkdf = authenticationHelper.getPasswordAuthenticationKey(this.username, authDetails.getPassword(), serverBValue, salt);

      const dateNow = dateHelper.getNowString();

      const signatureString = util.crypto.hmac(hkdf, util.buffer.concat([new util.Buffer(this.pool.getUserPoolId().split('_')[1], 'utf8'), new util.Buffer(this.username, 'utf8'), new util.Buffer(challengeParameters.SECRET_BLOCK, 'base64'), new util.Buffer(dateNow, 'utf8')]), 'base64', 'sha256');
      ...
      const respondToAuthChallenge = (challenge, challengeCallback) => this.client.makeUnauthenticatedRequest('respondToAuthChallenge', challenge, (errChallenge, dataChallenge) => {
  ...
  */
  ...
  getDeviceResponse(callback) {
    ...
    this.client.makeUnauthenticatedRequest('respondToAuthChallenge', {
      ...
      const hkdf = authenticationHelper.getPasswordAuthenticationKey(this.deviceKey, this.randomPassword, serverBValue, salt);

      const dateNow = dateHelper.getNowString();

      const signatureString = util.crypto.hmac(hkdf, util.buffer.concat([new util.Buffer(this.deviceGroupKey, 'utf8'), new util.Buffer(this.deviceKey, 'utf8'), new util.Buffer(challengeParameters.SECRET_BLOCK, 'base64'), new util.Buffer(dateNow, 'utf8')]), 'base64', 'sha256');
