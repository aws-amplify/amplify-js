// This module fixes sourcemap paths for the various Amplify packages. 
// It's needed because packages at build time live in different (relative) folder 
// locations than the folders where packages are installed from npm. 
// For example, /packages/aws-amplify/src/index.ts imports '@aws-amplify/ui'.
// At build time, the aws-amplify and amplify-ui folders are siblings, but when 
// the aws-amplify package is installed from npm, the amplify-ui folder is 
// installed at ./node_modules/@aws-amplify/ui, which is a new name and is 
// no longer a sibling to ./node_modules/aws-amplify like it was at build time. 
// These changes mean that end users can't easily debug into Amplify code, especially
...
// 1. sourcemap paths that point to node_modules dependencies (e.g. lodash)
//    are mapped to webpack:///./node_modules/*
// 2. sourcemap paths that point to sibling packages under the @aws-amplify alias 
//    (like the UI example above) are mapped to webpack:///./node_modules/@aws-amplify/*
//
// These path mappings are designed to be compatible with VSCode's default source mapping
// options here: https://github.com/Microsoft/vscode-chrome-debug#sourcemaps
//
// IMPORTANT: if new packages are added to Amplify, they should be added to the map below.
