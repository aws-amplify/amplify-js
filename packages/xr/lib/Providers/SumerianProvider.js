...
var SumerianProvider = /** @class */ (function(_super) {
	...
	SumerianProvider.prototype.loadScene = function(
		sceneName,
		domElementId,
		sceneOptions
	) {
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					...
					case 2:
						...
						request = core_1.Signer.sign(
							{ method: 'GET', url: sceneUrl },
							accessInfo,
							serviceInfo
						);
						fetchOptions.headers = __assign(
							__assign({}, fetchOptions.headers),
							request.headers
						);
						url = request.url;
						return [3 /*break*/, 4];
					...
					case 6:
						...
						if (apiResponse.status === 403) {
							if (apiResponseJson.message) {
								...
								throw new Errors_1.XRSceneLoadFailure(
									'Failure to authenticate user: ' + apiResponseJson.message
								);
							} else {
					...
					case 13:
						...
						// Log scene warnings
						for (
							_i = 0, _a = sceneController.sceneLoadWarnings;
							_i < _a.length;
							_i++
						) {
							warning = _a[_i];
	...
	SumerianProvider.prototype.getSceneController = function(sceneName) {
		...
		if (!sceneController) {
			var errorMsg =
				"Scene controller for '" + sceneName + "' has not been loaded";
			logger.error(errorMsg);
	...
	SumerianProvider.prototype.onSceneEvent = function(
		sceneName,
		eventName,
		eventHandler
	) {
