...
export class CommonSQLiteAdapter implements StorageAdapter {
	...
	async save<T extends PersistentModel>(
		...
		for await (const resItem of connectionStoreNames) {
			...
			const [resItemQueryStatement, resItemParams] = queryByIdStatement(
				id,
				modelName,
			);
			const fromDBForResItem = await this.db.get(
				resItemQueryStatement,
				resItemParams,
			);

			const opType: OpType =
				fromDBForResItem === undefined ? OpType.INSERT : OpType.UPDATE;

			const saveStatement = fromDBForResItem
	...
	async batchSave<T extends PersistentModel>(
		...
		for (const item of items) {
			...
			const { instance } = connectedModels.find(
				({ instance: instanceOfConnectedModel }) =>
					instanceOfConnectedModel.id === id,
			);
