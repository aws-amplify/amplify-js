...
import { ID, USER, extractTargetNamesFromSrc } from '~/src/util';
...
class MutationProcessor {
	...
	constructor(
		...
		private readonly MutationEventConstructor: PersistentModelConstructor<MutationEvent>,
	...
	> {
		return retry(
			async (
				retryModel: string,
				retryOperation: TransformerMutationType,
				retryData: string,
				retryCondition: string,
				retryModelConstructor: PersistentModelConstructor<PersistentModel>,
				retryMutationEventConstructor: PersistentModelConstructor<MutationEvent>,
				retryMutationEvent: MutationEvent,
			) => {
				...
				const opType = this.opTypeFromTransformerOperation(retryOperation);
						...
						if (err.errors && err.errors.length > 0) {
							...
							if (error.errorType === 'ConflictUnhandled') {
								...
								if (attempt > MAX_ATTEMPTS) {
									...
									try {
										retryWith = await this.conflictHandler!({
											modelConstructor: retryModelConstructor,
											localModel: this.modelInstanceCreator(
												retryModelConstructor,
												variables.input,
											),
										...
										logger.warn('conflict trycatch', _error);
										continue;
									}
								}
								...
								if (retryWith === DISCARD) {
									...
									const [[, retryOpName, retryQuery]] = buildGraphQLOperation(
										this.schema.namespaces[namespaceName],
										modelDefinition,
										'GET',
									);

									const retryAuthToken = await getTokenForCustomAuth(
										authMode,
										this.amplifyConfig,
									);
									...
									return [serverData, retryOpName, modelDefinition];
								}
								...
								const updatedMutation =
									createMutationInstanceFromModelOperation(
										...
										retryWith,
										graphQLCondition,
										retryMutationEventConstructor,
										this.modelInstanceCreator,
										retryMutationEvent.id,
									);
								...
								try {
									...
									logger.warn('Mutation error handler failed with:', _error);
								} finally {
