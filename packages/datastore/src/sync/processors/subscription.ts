...
class SubscriptionProcessor {
						...
						.forEach(
							modelDefinition =>
								...
								this.runningProcesses.add(async () => {
									...
									const addFilterArg = this.isValidRTF(
										modelDefinition,
										predicatesGroup
									);
									...
									// 		`this.isValidRTF()` will validate the expr. and prevent sending one we know will fail
	...
	/**
	 * @returns true if the filter expression can be used as a valid RTF expr.
	 * @remarks
	 * This does not validate against service limits; only unchanging intrinsic RTF rules:
	 * 1. can't use the same field more than once in the same group (in an AND expr.)
	 * 2. `not` group is not supported
	 *
	 */
	private isValidRTF(
		modelDefinition: SchemaModel,
		...
		const isNotGroupUsed = notGroupPredicateUsed(predicatesGroup);
	...
	private catchRTFError(
		...
		// Schema was generated by an older, pre-RTF CLI version. Subscription query is lacking $filter arg
		if (message.includes('UnknownArgument: Unknown field argument filter')) {
			warningMsg =
				`${message}\n` +
		...
		} else if (message.includes('Filters exceed maximum attributes limit')) {
			const selSyncFields = filterFields(predicatesGroup);
			const selSyncFieldStr = [...selSyncFields].reduce((sum, cur, idx) => {
				if (idx === 0) {
					return cur;
				}
				return `${sum}, ${cur}`;
			...
			const dynamicAuthFieldsStr = [...dynamicAuthModeFields].reduce(
				(sum, cur, idx) => {
					if (idx === 0) {
						return cur;
					}
			...
			if (dynamicAuthModeFields.size > 0) {
				warningMsg +=
					...
					`You currently have ${dynamicAuthModeFields.size} dynamic auth modes configured on this model: ${dynamicAuthFieldsStr}.`;
			}
		} else if (message.includes('Filters combination exceed maximum limit')) {
