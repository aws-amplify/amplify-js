...
var AmazonPersonalizeProvider = /** @class */ (function() {
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					...
					case 1:
						...
						requestParams = this.generateRequestParams(
							params,
							this._sessionInfo
						);
	...
	AmazonPersonalizeProvider.prototype.isElementFullyLoaded = function(
		operation,
		params,
		delay,
		times
			...
			return operation(params)
				...
				.catch(function(reason) {
					if (times - 1 > 0) {
						return wait(delay)
							.then(
								_this.isElementFullyLoaded.bind(
									null,
									operation,
									params,
									delay,
	 ...
	 */
	AmazonPersonalizeProvider.prototype.generateRequestParams = function(
		params,
		sessionInfo
	) {
		var requestParams = {};
	...
	AmazonPersonalizeProvider.prototype._sendEvents = function(group) {
		...
		if (groupLen > 0) {
			...
			for (var i = 0; i < groupLen; i += 1) {
				...
				var eventPayload = this._generateSingleRecordPayload(
					params,
					sessionInfo
				);
	...
	AmazonPersonalizeProvider.prototype._sendFromBuffer = function() {
		...
		for (var i = 0; i < size; i += 1) {
			...
			if (i === 0) {
				...
				if (
					lodash_1.isEqual(sessionInfo, this._sessionInfo) &&
	 ...
	 */
	AmazonPersonalizeProvider.prototype._generateSingleRecordPayload = function(
		params,
		sessionInfo
	) {
