	...
	EventTracker.prototype.configure = function(opts) {
		...
		if (!this._config.enable) {
			...
			this._config.events.forEach(function(evt) {
				_this._delegates[evt] = dom_utils_1.delegate(
					document,
					evt,
					selector_1,
					_this._trackFunc,
				...
				switch (_b.label) {
					case 0:
						...
						events = element
							...
							.split(/\s*,\s*/);
						eventName = element.getAttribute(
						...
						if (!(typeof this._config.attributes === 'function'))
							return [3 /*break*/, 2];
					...
					case 3:
						...
						if (events.indexOf(event.type) < 0) {
							logger.debug(
								'event ' + event.type + ' is not selected to be recorded'
							);
							return [2 /*return*/];
						}
						this._tracker(
							...
							logger.debug(
								'Failed to record the ' + event.type + " event', " + e
							);
						});
						return [2 /*return*/];
				}
