...
var MqttOverWSProvider = /** @class */ (function(_super) {
		...
		var _this =
			_super.call(
				...
				__assign(__assign({}, options), {
					clientId: options.clientId || uuid_1.v4(),
				})
			) || this;
	...
	MqttOverWSProvider.prototype.onDisconnect = function(_a) {
		...
		if (errorCode !== 0) {
			logger.warn(
				clientId,
				JSON.stringify(__assign({ errorCode: errorCode }, args), null, 2)
			);
			...
			clientIdObservers.forEach(function(observer) {
				...
				_this._topicObservers.forEach(function(
					observerForTopic,
					observerTopic
				) {
				...
				switch (_b.label) {
					case 0:
						...
						client.onConnectionLost = function(_a) {
							...
							_this.onDisconnect(
								__assign({ clientId: clientId, errorCode: errorCode }, args)
							);
						};
						return [
							4 /*yield*/,
						...
						return [
							...
							this.clientsQueue.get(clientId, function(clientId) {
								return _this.newClient(
									__assign(__assign({}, options), { clientId: clientId })
								);
							}),
						];
					case 1:
		...
		return new Observable(function(observer) {
			...
			return function() {
				...
				if (client) {
					...
					targetTopics.forEach(function(topic) {
						var observersForTopic =
							_this._topicObservers.get(topic) || new Set();
