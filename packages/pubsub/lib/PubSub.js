	...
	Object.defineProperty(PubSub.prototype, 'awsAppSyncRealTimeProvider', {
		...
		get: function() {
			if (!this._awsAppSyncRealTimeProvider) {
				this._awsAppSyncRealTimeProvider = new Providers_1.AWSAppSyncRealTimeProvider(
					this._options
				);
			}
		...
		return __awaiter(this, void 0, void 0, function() {
			return __generator(this, function(_a) {
				return [
					2 /*return*/,
					Promise.all(
						this.getProviders(options).map(function(provider) {
		...
		return new Observable(function(observer) {
			...
			var subscriptions = observables.map(function(_a) {
				...
				return observable.subscribe({
					start: console.error,
					next: function(value) {
						return observer.next({ provider: provider, value: value });
					},
					error: function(error) {
						return observer.error({ provider: provider, error: error });
					},
				});
			});
			return function() {
