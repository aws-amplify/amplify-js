...
import { AmplifyRTNPushNotification, PushNotificationNativeModule } from '@aws-amplify/rtn-push-notification';

import { addEventListener, EventListener, notifyEventListeners, notifyEventListenersAndAwaitHandlers } from '../common';
...
export default class PushNotification implements PushNotificationInterface {
	...
	constructor() {
		try {
			...
			const { NativeEvent, NativeHeadlessTaskKey } = this.nativeModule.getConstants();
	...
	addPluggable = (pluggable: PushNotificationProvider): void => {
		if (pluggable && pluggable.getCategory() === 'Notifications' && pluggable.getSubCategory() === 'PushNotification') {
	...
	enable = (): void => {
		...
		if (this.nativeHeadlessTaskKey) {
			...
			// library listeners
			AppRegistry.registerHeadlessTask(this.nativeHeadlessTaskKey, () => async message => {
			...
			this.nativeEventEmitter.addListener(BACKGROUND_MESSAGE_RECEIVED, async message => {
				...
				try {
					...
					this.nativeModule.completeNotification?.(message.completionHandlerId);
				}
			});
		}
		...
		this.nativeEventEmitter.addListener(
			...
			FOREGROUND_MESSAGE_RECEIVED,
			message => {
				notifyEventListeners(PushNotificationEvent.FOREGROUND_MESSAGE_RECEIVED, normalizeNativeMessage(message));
			}
		);
		...
		const launchNotificationOpenedListener = LAUNCH_NOTIFICATION_OPENED
			? this.nativeEventEmitter.addListener(
					...
					LAUNCH_NOTIFICATION_OPENED,
					message => {
						notifyEventListeners(PushNotificationEvent.LAUNCH_NOTIFICATION_OPENED, normalizeNativeMessage(message));
		...
		this.nativeEventEmitter.addListener(
			...
			NOTIFICATION_OPENED,
			message => {
				notifyEventListeners(PushNotificationEvent.NOTIFICATION_OPENED, normalizeNativeMessage(message));
	...
	getLaunchNotification = async (): Promise<PushNotificationMessage | null> => {
		this.assertIsEnabled();
		return normalizeNativeMessage(await this.nativeModule.getLaunchNotification?.());
	};
	...
	getPermissionStatus = async (): Promise<PushNotificationPermissionStatus> => {
		this.assertIsEnabled();
		return normalizeNativePermissionStatus(await this.nativeModule.getPermissionStatus?.());
	};
	...
	onNotificationReceivedInBackground = (
		...
		this.assertIsEnabled();
		return addEventListener(PushNotificationEvent.BACKGROUND_MESSAGE_RECEIVED, handler);
	};
	...
	onNotificationReceivedInForeground = (
		...
		this.assertIsEnabled();
		return addEventListener(PushNotificationEvent.FOREGROUND_MESSAGE_RECEIVED, handler);
	};
