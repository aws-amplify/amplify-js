(function($) {
	...
	if (
		...
		if (ref_url && ref_content) {
			...
			$('.installation_custom_next_step').html(
				"Continue following the <a href='" +
					ref_url +
					'#' +
					ref_content_section +
					"'>" +
	...
	$('ul.tabs li').click(function(event, stopPropogation) {
		...
		if (stopPropogation == true) return;
	...
	$('div.row.notification-bar a').click(function(event) {
	...
	var showNotificationBar = function(messageDate) {
		...
		if (Cookies.get('notificationMessage_LastReceived')) {
			lastMessageReceived = new Date(
				Cookies.get('notificationMessage_LastReceived')
			);
		} else {
	...
	var expandSearchBar = function() {
		...
		document.getElementsByClassName('offcanvas-toggle')[0].style.visibility =
			'hidden';
	};
